...
えー、とりあえず写したらバグって困ってます。原因を見つけるのがリファクタリングするのよりしんどいので、一旦ここで終わりにしようと思います。色々なところでメモリを書き換えているので、読むのがしんどいです。

なので、ページ（ノード）を[PAGE_SIZE]byteで持つのではなくて、構造体で持つようにしたいです。ファイルへの永続化は`.exit`でまとめてやっているので、そうしても問題ないはずです。
